...
maxIter = 300000;
stepSize = 4e-3;
funObj = @(w,i)MLPclassificationLoss(w,X(max(1,i-3):i,:),yExpanded(max(1,i-3):i,:),nHidden,nLabels);
iteration = [];
error = [];
key = 1;
...
% Evaluate test error
yhat = MLPclassificationPredict(w_optimal,Xtest,nHidden,nLabels);
